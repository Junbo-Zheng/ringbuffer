...
uint32_t is_power_of_2(uint32_t n) { return (n != 0 && ((n & (n - 1)) == 0)); }

void ringbuffer_init(struct ringbuffer* ring_buf, uint8_t* buf,
                     uint32_t buf_size)
{
    if (!is_power_of_2(buf_size)) return;
    ring_buf->data = buf;
...
uint32_t ringbuffer_put(struct ringbuffer* ring_buf, uint8_t* buffer,
                        uint32_t len)
{
    uint32_t l;
    ...
    memcpy((uint8_t*)(ring_buf->data)
               + (ring_buf->write_pos & (ring_buf->size - 1)),
           buffer, l);
 ...
 *  writer, you don't need extra locking to use these functions.
 */
uint32_t ringbuffer_get(struct ringbuffer* ring_buf, uint8_t* buffer,
                        uint32_t len)
{
    uint32_t l;
    ...
    memcpy(buffer,
           (uint8_t*)(ring_buf->data)
               + (ring_buf->read_pos & (ring_buf->size - 1)),
           l);
 ...
 *
 * Note: usage of ringbuffer_reset() is dangerous. It should be only called when
 * the ringbuffer is exclusived locked or when it is secured that no other
